...
<div class="d-flex mb-2">
    ...
    <div class="d-inline-block editor-action-item">
        <a href="#" class="btn_gallery btn btn-primary" data-result="{{ $result }}" data-multiple="true" data-action="media-insert-{{ BaseHelper::getRichEditor() }}">
    ...
    <div class="d-inline-block editor-action-item list-shortcode-items">
        <div class="dropdown">
            ...
            <ul class="dropdown-menu">
                ...
                @if ($item['name'])
                <li>
                    <a href="{{ route('short-codes.ajax-get-admin-config', $key) }}" data-has-admin-config="{{ Arr::has($item, 'admin_config') }}" data-key="{{ $key }}" data-description="{{ $item['description'] }}" data-preview-image="{{ $item['previewImage'] }}">{{ $item['name'] }}</a>
                </li>
                @endif
                @endforeach
            </ul>
        ...
        <div class="modal-dialog modal-md">
            <div class="modal-content">
                <div class="modal-header bg-primary">
                    ...
                    <div class="float-end">
                        <a class="shortcode-preview-image-link bold color-white" style="color: white" target="_blank" href="">{{ __('View preview image') }}</a>
